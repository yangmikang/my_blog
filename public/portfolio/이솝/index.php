<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>이솝 대한민국</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css">
  
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

  <!-- 아울 캐러셀 불러오기 -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/owl.carousel.min.js"></script>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.carousel.min.css">
  <link rel="stylesheet"
    href="https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.3.4/assets/owl.theme.default.min.css">

  <link rel="stylesheet" href="./index.css">
  <script src="./index.js"></script>
  <!--급할땐 이부분을 바디 젤아래로 -->
</head>

<body>

  <div class="top-dv-bar con">
    <a href="#">이솝 공식 홈페이지를 이용해 주셔서 감사합니다. 주문 폭주 및 배송사의 사정으로 배송 지연이 발생 될 수 있는 점 참고 부탁 드립니다.</a>
  </div>
  <div class="shopping-bar con">
    <a href="#">전 제품 무료 배송 혜택을 즐겨보세요.
      <svg aria-labelledby="97f116fc-ff6f-4516-9a3d-054d1ee26097" class="Icon ShippingBanner-openModalIcon" role="img"
        viewBox="0 0 50 50">
        <title id="97f116fc-ff6f-4516-9a3d-054d1ee26097">Open button for Shipping in modal</title>
        <g>
          <polygon class="Glyph-addWithCircle--plus"
            points="26.2,15.2 23.8,15.2 23.8,23.9 15,23.9 15,26.4 23.8,26.4 23.8,35.1 26.2,35.1 26.2,26.4 35,26.4 35,23.9 26.2,23.9 ">
          </polygon>
        </g>
      </svg>
    </a>
  </div>
  <div class="bn-bar con active-on-visible"
    style="background-image:url(https://www.aesop.com/medias/1Aesop-Citrus-Melange-Body-Cleanser-2020-Hybris-Homepage-Primary-Full-Bleed-Desktop-4000x1450px.jpg?context=bWFzdGVyfGltYWdlc3w1NTg0NDJ8aW1hZ2UvanBlZ3xpbWFnZXMvaGE4L2gyNC85NDQzODk4ODg0MTI2LmpwZ3xjYzE4NDVhNmZkMDM0M2QxZDJiNGE4ZDkwNzI4ZGM1MmQxNjg0NzYxM2JjOGQ2OTFkNDhiZjI0MjViOGJlYTlm);">
    <div class="bn-top-bar">
      <div class="top-box flex flex-1-0-0">
                <ul class=" flex flex-1-0-0">
        <li><a href="#">제품보기</a></li>
        <li><a href="#">읽기</a></li>
        <li><a href="#">스토어</a></li>
        <li><a href="#">검색</a></li>
        </ul>
        <ul class="login flex con">
          <li class="flex"><a href="#">로그인</a></li>
          <li class="flex"><span></span></li>
        </ul>
      </div>
    </div>
  </div>
  <div class="aesop-logo active-on-visible">
    <svg aria-labelledby="f025ef54-c189-4c11-93b7-36963e132cea" class="Icon NavLogo-icon" role="img"
      viewBox="0 0 489.7 154.3">
      <title id="f025ef54-c189-4c11-93b7-36963e132cea">Aesop logo</title>
      <g>
        <path class="NavLogo-icon--path"
          d="M48.6,22.5L31.8,62.2h34.2L48.6,22.5z M28.6,70.3c-3.2,7.4-9.6,21.7-14.9,42c-1.9-0.3-3.5-0.9-6.6-0.9c-3.1,0-5,0.6-7.1,0.9C14.3,81.2,38,28.9,49.4,0c1.3,0.2,2.3,0.5,4,0.5c1.4,0,2.3-0.2,3.5-0.5c12.3,33.7,47.8,111.7,48.1,112.3c-2.7-0.5-5.5-0.9-9.5-0.9c-4.2,0-7.2,0.5-10.3,0.9c-5.6-19.2-12.2-33.2-16.2-42H28.6z">
        </path>
        <path class="NavLogo-icon--path"
          d="M172.4,66.6c0-5.3,0-26.6-19.2-26.6c-14,0-20.7,10.6-22.2,26.6H172.4z M130.9,73.3c0,4.5,0,13.6,4.3,21c7.4,12.1,17.8,12.5,22.5,12.5c15.7,0,23.1-9.6,25.5-13l1.9,0.4c-1,4.5-1,7.1-1.1,9.6c-7.1,6.7-16.8,10.5-29.8,10.5c-24.4,0-39.3-15.4-39.3-40.1c0-22.1,12.2-40.7,38.5-40.7c35.8,0,35.3,31.1,35.1,39.8H130.9z">
        </path>
        <path class="NavLogo-icon--path"
          d="M249.6,52.1c-0.6-2.4-1.6-6.4-6.2-9.6c-3.4-2.4-7.8-2.7-10.4-2.7c-9.8,0-14.9,5.1-14.9,12c0,9.3,9.5,12.7,15.1,14.1c14.3,3.7,25.8,6.7,25.8,22.8c0,12.2-8.5,25.8-30.8,25.8c-13,0-22-5.5-23.7-6.7c2.1-3.9,3.4-10.7,3.7-12.8l1.9-0.5c1.1,2.7,2.7,7.1,8.7,10.6c4,2.4,8.2,3,11.7,3c8.4,0,16.2-4,16.2-13.2c0-9.1-5.5-10.9-22.3-16.4c-6.9-2.3-18.3-7.2-18.3-21.3c0-9.1,5.6-23.9,27.8-23.9c11.1,0,17.5,3.4,21.2,5.3c-1.6,4.2-2.7,8-3.2,13.5H249.6z">
        </path>
        <path class="NavLogo-icon--path"
          d="M291.5,73.5c0,22.3,9.8,34.6,24.5,34.6c15.4,0,25.5-13.3,25.5-34.5c0-12.5-3.9-33.8-24.7-33.8C291.5,39.8,291.5,68.6,291.5,73.5 M357.9,73c0,25.5-17.6,41.5-42,41.5c-17,0-40.8-7.7-40.8-40.6c0-25,16.7-40.6,40.8-40.6 C346.3,33.4,357.9,53.4,357.9,73">
        </path>
        <path class="NavLogo-icon--path"
          d="M399.4,49.6c-3.2,4-6.6,9.8-6.6,25.7c0,14.1,3.2,19.4,6.6,23.4c4.1,5,9.6,7.9,17,7.9 c22.9,0,22.9-27.1,22.9-32.3c0-24.2-11.6-33-22-33C408.7,41.2,403.1,44.9,399.4,49.6 M419.3,114.7c-15.1,0-23.6-8.3-26.2-14.9 c0,25.2-0.2,39.8,0.8,54.5c-2.4-0.6-4.5-0.9-8.4-0.9c-4,0-6.1,0.3-8.6,0.9c1.3-20.8,1.8-41.9,1.8-62.7c0-14.6-0.3-34.3-0.8-57.6 c2.2,0.5,4.8,0.9,7.9,0.9c2.9,0,5.6-0.5,7.9-0.9c-0.6,8.8-0.6,11.2-0.6,14.6c2.6-4.2,9.5-15.4,27.3-15.4c16,0,35.6,11.7,35.6,40.3 C455.9,98.8,440.3,114.7,419.3,114.7">
        </path>
        <rect x="131.8" y="13.5" class="NavLogo-icon--path" width="44.3" height="5"></rect>
        <path class="NavLogo-icon--path"
          d="M477.2,98.1c0,1.6,0,3,0,4.4c1.3,0,4.3,0,4.3-2.3C481.5,98.1,479.2,98.1,477.2,98.1 M484.9,109.1 c-0.4,0-0.7-0.1-1.2-0.1c-0.4,0-0.8,0.1-1.5,0.1c-2-2.9-3.6-4.5-4.7-5.8h-0.4c0,1.8,0,3.2,0.2,5.8c-0.5,0-0.7-0.1-1.3-0.1 c-0.6,0-0.8,0.1-1.1,0.1c0.1-1.3,0.2-2.7,0.2-6.2c0-3-0.1-4.4-0.1-5.7c0.9,0,1.5,0,2.9,0c2.9,0,5.9,0,5.9,2.8c0,2.5-2.7,2.9-3.9,3 c0.8,1.1,4.4,5.1,5.1,5.9L484.9,109.1z M479.2,94.8c-4.8,0-8.5,3.8-8.5,8.6c0,4.9,3.6,8.6,8.5,8.6c4.9,0,8.5-3.7,8.5-8.6 C487.8,98.7,484.3,94.8,479.2,94.8 M479.2,92.9c5.9,0,10.5,4.7,10.5,10.5c0,5.9-4.7,10.5-10.5,10.5c-5.8,0-10.5-4.6-10.5-10.5 C468.8,97.5,473.5,92.9,479.2,92.9">
        </path>
      </g>
    </svg>
  </div>
  <div class="bn-text-bar active-on-visible">
    <div class="text-bar">
      <h5>갓 짜낸 신선함</h5>
      <h1>시트러스 멜란지 바디 클렌저 </h1>
      <h5>
        새롭게 선보이는 바디 클렌저는 기분을 고양시키는 페티그레인(비터오렌지잎/잔가지오일), 그레이프프루트, 레몬의 블렌드로 이루어진 상쾌한 샤워 젤로 세정 루틴에 변화를 선사합니다.
      </h5>
      <div class="view-bt">
        <a href="#"> 지금 보기</a>
        <i class="fas fa-arrow-right"></i>
      </div>


    </div>
  </div>


  <div class="my-slider-1 con-3 active-on-visible">
    <div class="owl-carousel owl-theme">
      <div class="item">
        <h4><img
            src="https://www.aesop.com/medias/Aesop-Body-Geranium-Leaf-Body-Cleanser-500mL-large.png?context=bWFzdGVyfGltYWdlc3w1OTg2NDd8aW1hZ2UvcG5nfGltYWdlcy9oNmIvaGYwLzg4MTQ5NzU0ODM5MzQucG5nfDZiMmQ1YWYzNmE3ZTBlYjZiYTI2Njg3MjBhODA5MDU4MDcxYjllNmI5NWE3NzU4MzhhZTY5MDM3MGNmMTY1MDI"
            alt="">
          <a href="#"><span>제라늄 리프 바디 클렌저</span> </a>
          <a href="#">상쾌함을 선사하는 부드러운 젤 타입 클렌저</a>
        </h4>

      </div>
      <div class="item">
        <h4><img
            src="https://www.aesop.com/medias/Aesop-Body-Rind-Concentrate-Body-Balm-120mL-large.png?context=bWFzdGVyfGltYWdlc3w1ODgyNjB8aW1hZ2UvcG5nfGltYWdlcy9oYzUvaDIxLzg3OTc0Mzk1OTA0MzAucG5nfDFiNGYxMGEzY2RlZGI4ZGJmZDQ5NDA0MmQ4NTZkZDlhYzI1OWNkYmNjNzhiYTZiMTM4MmY4MTNhNmNiYjVlZjY"
            alt="">
          <a href="#"><span>라인드 컨센트레이트 바디 밤</span></a><a href="#">빠른 흡수, 가벼운 수분 공급</a></h4>
      </div>
      <div class="item">
        <h4><img
            src="https://www.aesop.com/medias/Aesop-Body-Redemption-Body-Scrub-180mL-large.png?context=bWFzdGVyfGltYWdlc3w2NTYxNDJ8aW1hZ2UvcG5nfGltYWdlcy9oNzAvaDA1Lzg3OTc0MzkxNjQ0NDYucG5nfDhiMzBmOTE2NjBkZmFjOWE4NzM0ZjEwNmEyZTdkNjVhNjk1NzY4ZWJiZmQ3ZjE2M2NlNzU0MDIxOWZiNjJhMmQ"
            alt="">
          <a href="#"><span>리뎀션 바디 스크럽</span></a><a href="#">밤부사 불가리스줄기추출물과 부석이 피부 표면을 매끄럽게 가꿔줍니다.</a></h4>
      </div>
      <div class="item">
        <h4><img
            src="https://www.aesop.com/medias/Aesop-Body-A-Rose-By-Any-Other-Name-Body-Cleanser-500mL-large.png?context=bWFzdGVyfGltYWdlc3w2MDAwNzN8aW1hZ2UvcG5nfGltYWdlcy9oMWUvaGUyLzg4MTQ1NjMxOTY5NTgucG5nfDRlNmVlNWFkY2U5MTdjYWQyMTk4YTg5NjUxMzRlNTY4M2Y0NmQ0NzRlNzA5YTExYzI3OWVkNjIzYmYyNzdkYjA"
            alt="">
          <a href="#"><span>에이 로즈 바이 애니 아더 네임 바디 클렌저</span></a><a href="#">부드럽게 세정하는 향기로운 바디 클렌저</a></h4>
      </div>
      <div class="item">
        <h3><img
            src="https://www.aesop.com/medias/Aesop-Geranium-Leaf-Duet-with-Product-Large-1584x962px.png?context=bWFzdGVyfGltYWdlc3w0MTIxMzV8aW1hZ2UvcG5nfGltYWdlcy9oZTEvaGYyLzkxMTA3OTg5NTg2MjIucG5nfDMxNDhlMjJkMWY4NDZiNDU5NWJjMDcxZmM4MzljMmYxNjZhMDVmMTEzMTI1NDZkMTk3ODcxNzA5YjdmNDhlMDU"
            alt="">
          <a href="#"><span>제라늄 리프 듀엣</span></a><a href="#">바디 관리를 위한 클렌징과 밤으로 구성된 세트</a></h3>
      </div>
      <div class="item">
        <h4><img
            src="https://www.aesop.com/medias/Aesop-Body-Coriander-Seed-Body-Cleanser-500mL-large.png?context=bWFzdGVyfGltYWdlc3w2MDQ3MTJ8aW1hZ2UvcG5nfGltYWdlcy9oYzIvaDkxLzg4MTQ1NjQzNDM4MzgucG5nfGUwOTMxNGM2NDI2MjIyMTBjYTc1NDg0NzQ0NTdhOTNiYWNiM2Q1YzI5N2I4OTM1NGU3MmYxNjkzODU3NTA0OTg"
            alt="">
            <a href="#"><span>코리안더 씨드 바디 클렌저</span></a><a href="#">따뜻한,스파이시향</a></h4>
      </div>
    </div>
  </div>

  





  <div class="slider-line con active-on-visible">
    <div class="cursor-line"></div>

  </div>



  <div class="about-item con active-on-visible">
    <div class="about-text con">
      <ul>
        <li>
          <div class="ab-title">
            <h5>페이셜 어포인트먼트</h5>
            <h1>피부와 심신의 안정</h1>
            <h5>균형잡힌 스킨케어 관리를 위해 이솝 페이셜어포인트먼트는 피부에 생기와 집중적인 영양을 공급하는 맞춤관리를 제공합니다.</h5>
            <div class="ab-view"><a href="#">더 알아보기

              </a>
              <i class="fas fa-arrow-right"></i>
            </div>
          </div>

        </li>
        <li>
          <div class="about-img active-on-visible">
            <img
              src="https://www.aesop.com/medias/GenericImages-FacialAppointment-Primary-Mid-Desktop-1690x1400px.jpg?context=bWFzdGVyfGltYWdlc3wxNjM4NDh8aW1hZ2UvanBlZ3xpbWFnZXMvaDc5L2g0Ni84ODc5NzQ1NTY0NzAyLmpwZ3wyOTBjMmVmZDk3Y2NlZmJlYWNlMjM5ODM2ZDE4MTAxYzc4YTM5MjQ3YjE4ZDQ0ZmQ2NWM1ZWU5Y2MxNTg1NmNi"
              alt="">
          </div>
        </li>

      </ul>
    </div>
  </div>
  <div class="about-item-2 con-2 active-on-visible">
    <div class="about-text">
      <ul>
        <li>
          <div class="ab-title">
            <h5>방문</h5>
            <h1>신규 스토어: 이솝 롯데 수지</h1>
            <h5>최근에 오픈한 이솝 스토어에서 소중한 사람을 위한 시즈널 기프트를 만나보세요.</h5>
            <div class="ab-view"><a href="#">스토어 구경하기

              </a>
              <i class="fas fa-arrow-right"></i>
            </div>
          </div>

        </li>
        <li>
          <div class="about-img">
            <img
              src="https://www.aesop.com/medias/Aesop-Suji-Carousel-1-Desktop-2560x1440px.jpg?context=bWFzdGVyfGltYWdlc3wzMzA5NjR8aW1hZ2UvanBlZ3xpbWFnZXMvaGRjL2hjYy85MTY4NzA1MzIzMDM4LmpwZ3w2MjEyNWM4YTM3MGUyMmI0NWQ5MWRhNTE5MGE3NmVjMDQyMmI0NTNmNDI0NTlmODA1N2JkZWIzNTgwNWVmZGI5"
              alt="">
            <h1>이솝 롯데 수지</h1>
          </div>
        </li>

      </ul>
    </div>
  </div>
  <div class="bottom-text con active-on-visible">
    <h1>‘Know'st thou the land where the lemon-trees bloom?’</h1>
    <h5> Johann Wolfgang von Goethe</h5>
  </div>
  <div class="copy-light con-2 flex">
    <div class=" text-bar-2 con-2 ">
      <h5>구독하기</h5>
      <h3>이메일 주소</h3>
      <h5>제품,스토어,이벤트,그리고 이솝이 고른 글들이 담긴 이솝 뉴스레터를 받고 싶습니다. &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</h5>
    </div>
    <div class="text-box">
      <ul>
        <li><a href="#">문의 하기</a>
      </li>
        <li><a href="#">문의하기</a></li>
        <li><a href="#">FAQS</a></li>
        <li><a href="#">배송 및 반품</a></li>
        <li><a href="#">배송 확인</a></a></li>
        <li><a href="#">이용 약관</a></li>
        <li><a href="#">개인정보 정책</a></li>
      </ul>
      <ul>
        <li><a href="#">브랜드 소개</a></li>
        <li><a href="#">브랜드 스토리</a></li>
        <li><a href="#">채용</a></li>
        <li><a href="#">마케팅 문의</a></li>
        <li><a href="#">기업용 선물</a></li>
        <li><a href="#">페이셜 어포인트먼트</a></li>
        <li><a href="#">개인 정보 보호 정책</a></li>
      </ul>
      <ul>
        <li><a href="#">소셜</a></li>
        <li><a href="#">instagram</a>
        <svg class="Icon FooterLinks-linkExternalIcon" role="img" viewBox="0 0 50 50">
													<title>External Link</title><g>
													<path d="M21.8,0.2l28.1-0.1v28.1l-4.5,0l0.1-20.5L16.2,37.1l-3.4-3.4L42.1,4.4L21.8,4.4L21.8,0.2L21.8,0.2z"></path></g></svg></li>
        <li><a href="#">Twitter</a>
        <svg class="Icon FooterLinks-linkExternalIcon" role="img" viewBox="0 0 50 50">
													<title>External Link</title><g>
													<path d="M21.8,0.2l28.1-0.1v28.1l-4.5,0l0.1-20.5L16.2,37.1l-3.4-3.4L42.1,4.4L21.8,4.4L21.8,0.2L21.8,0.2z"></path></g></svg>
      </li>
        <li><a href="#">Linkedin</a>
        <svg class="Icon FooterLinks-linkExternalIcon" role="img" viewBox="0 0 50 50">
													<title>External Link</title><g>
													<path d="M21.8,0.2l28.1-0.1v28.1l-4.5,0l0.1-20.5L16.2,37.1l-3.4-3.4L42.1,4.4L21.8,4.4L21.8,0.2L21.8,0.2z"></path></g></svg></li>
        <li><a href="#">KaKao Plus Friend</a>
        <svg class="Icon FooterLinks-linkExternalIcon" role="img" viewBox="0 0 50 50">
													<title>External Link</title><g>
													<path d="M21.8,0.2l28.1-0.1v28.1l-4.5,0l0.1-20.5L16.2,37.1l-3.4-3.4L42.1,4.4L21.8,4.4L21.8,0.2L21.8,0.2z"></path></g></svg></li>
        <li><a href="#">Naver Post</a>
        <svg class="Icon FooterLinks-linkExternalIcon" role="img" viewBox="0 0 50 50">
													<title>External Link</title><g>
													<path d="M21.8,0.2l28.1-0.1v28.1l-4.5,0l0.1-20.5L16.2,37.1l-3.4-3.4L42.1,4.4L21.8,4.4L21.8,0.2L21.8,0.2z"></path></g></svg></li>
      </ul>
    </div>

    <div class=""></div>
  </div>
  <div class="about-bottom-text">
    <div class="about"><a href="#">상호: 이솝코리아 유한회사 | 주소: 서울특별시 강남구 선릉로 162길 3 이솝코리아 3층 | 대표자: 마이클안토니오키프 | 대표전화: 1800-1987
        | 대표 이메일:
        aesop@aesop.com | 호스팅 사업자: SAP Hybris | 사업자 등록 번호: 220-88-56014 | 통신판매업 신고번호: 2014-서울강남-02300</a></div>
  </div>
<div class="foot-box active-on-visible">
  <div class="company">© 이솝
</div>
<div class="langs"><a href="#">대한민국</a></div>

</div>

</body>

</html>
